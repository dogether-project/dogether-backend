...
export default function (data) {
    ...
    check(null, {
        ...
        '응답 데이터 - todos[0].certificationContent 존재': () => responseData?.todos[0].certificationContent !== undefined,
        '응답 데이터 - todos[0].certificationMediaUrl 존재': () => responseData?.todos[0].certificationMediaUrl !== undefined,
    });
}
...
function requestApi(vuIndex, challengeGroupId) {
    ...
    return http.get(`${API_BASE_URL}/api/v1/challenge-groups/${challengeGroupId}/my-todos?date=${todayDate}&status=REVIEW_PENDING`, { headers, timeout });
}
