...
export function createCurrentActivityData() {
    ...
    for (let i = 0; i < TOTAL_CHALLENGE_GROUP_COUNT + (MEMBER_COUNT / MEMBER_PER_GROUP_COUNT); i++) {
            ...
            for (let i = 0; i < GROUP_PER_MEMBER_COUNT; i++) {
                ...
                for (let j = 0; j < DAY_TODO_PER_MEMBER_COUNT; j++) {
                    ...
                    if (day === CURRENT_GROUP_RUNNING_DAY - 1 && i === 1) {
                        daily_todo_data.push([currentTodoId, groupIdsByMember[memberId - 1][i], memberId, `td=${currentTodoId}`, "CERTIFY_PENDING", currentTodoWrittenAt, todayDate, null]);
                    ...
                    if (day === CURRENT_GROUP_RUNNING_DAY - 1 && i === 2) {
                        daily_todo_certification_data.push([currentTodoCertificationId, currentTodoId, `tc-${currentTodoId}`, `http://certification-media.site/m${memberId}/t${currentTodoId}`, 'REVIEW_PENDING', null, currentTodoCertifyAt, todayDate, null]);
...
export function getJoinCodesPerMember() {
    ...
    for (let i = 0; i < MEMBER_COUNT / MEMBER_PER_GROUP_COUNT; i++) {
        const currentChallengeGroupId = challengeGroupId + TOTAL_CHALLENGE_GROUP_COUNT + i;
